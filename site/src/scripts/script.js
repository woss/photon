...
console.log(
	"%c Hire the dev behind Photon -> silvi.dev",
...
function drawChart() {
	var myChart = new Chart(ctx, {
		type: "horizontalBar",
		data: {
			labels: ["Python Imaging Lib", "ImageMagick", "Photon"],
			datasets: [
				{
					label: "# of Seconds",
					data: [40, 22, 2],
...
import("../../../crate/pkg").then((module) => {
	...
	function applyEffect(filter_name, in_canvas, in_ctx, in_img) {
		...
		let filter_dict = {
			...
			red_channel_grayscale: function () {
				single_channel_grayscale(rust_image, 0);
			},
			green_channel_grayscale: function () {
				single_channel_grayscale(rust_image, 1);
			},
			blue_channel_grayscale: function () {
				single_channel_grayscale(rust_image, 2);
			},
			hue_rotate_hsl: function () {
				colour_space(rust_image, "hsl", "shift_hue");
			},
			hue_rotate_hsv: function () {
				colour_space(rust_image, "hsv", "shift_hue");
			},
			hue_rotate_lch: function () {
				colour_space(rust_image, "lch", "shift_hue");
			},
			lighten_hsl: function () {
				colour_space(rust_image, "hsl", "lighten");
			},
			lighten_hsv: function () {
				colour_space(rust_image, "hsv", "lighten");
			},
			lighten_lch: function () {
				colour_space(rust_image, "lch", "lighten");
			},
			darken_hsl: function () {
				colour_space(rust_image, "hsl", "darken");
			},
			darken_hsv: function () {
				colour_space(rust_image, "hsv", "darken");
			},
			darken_lch: function () {
				colour_space(rust_image, "lch", "darken");
			},
			desaturate_hsl: function () {
				colour_space(rust_image, "hsl", "desaturate");
			},
			desaturate_hsv: function () {
				colour_space(rust_image, "hsv", "desaturate");
			},
			desaturate_lch: function () {
				colour_space(rust_image, "lch", "desaturate");
			},
			saturate_hsl: function () {
				colour_space(rust_image, "hsl", "saturate");
			},
			saturate_hsv: function () {
				colour_space(rust_image, "hsv", "saturate");
			},
			saturate_lch: function () {
				colour_space(rust_image, "lch", "saturate");
			},
		...
		module.putImageData(in_canvas, in_ctx, rust_image);
	}
	...
	function setUpTimeouts() {
		...
		for (let k = 0; k < landing_effects.length; k++) {
			...
			setTimeout(function () {
				applyEffect(effect, canvas, ctx, img);
			}, timer);
			timer += 1000;
			timer -= time_dec;
			if (time_dec > 900) {
		...
		setInterval(function () {
			...
			for (let k = 0; k < correction_effects.length; k++) {
				...
				setTimeout(function () {
					applyEffect(effect, correction_canvas, correction_ctx, img2);
				}, timer);
				timer += 1000;
			}
		}, 10000);
		...
		setInterval(function () {
			...
			for (let k = 0; k < effects.length; k++) {
				...
				setTimeout(function () {
					applyEffect(effect, effects_canvas, effects_ctx, img3);
				}, timer);
				timer += 500;
			}
		}, 4000);
