...
import("@silvia-odwyer/photon").then((module) => {
	...
	function watermark() {
		...
		let watermark_photon_img = module.open_image(
			watermark_canvas,
			watermark_ctx,
		);
	...
	function blendImages(event) {
		...
		let filter_dict = {
			...
			watermark: function () {
				return module.watermark(canvas, watermark_canvas, 10, 30);
			},
		};
	...
	function resize(event) {
		...
		let newcanvas = module.resize_img_browser(
			photon_img,
			360,
			239,
			resize_type,
	...
	function applyEffect(event) {
		...
		let filter_dict = {
			...
			duotone: function () {
				return module.duotone(rust_image, rgb1, rgb2);
			},
			lix: function () {
